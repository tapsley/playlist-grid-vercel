...
const appList = [
    {
        ...
        title: "Trivia Game",
        version: "1.0.0",
        path: `${BASE_PATH}/apps/TriviaGame.zip`,
        icon: `${BASE_PATH}/images/icons/trivia-game-icon.png`,
    },
    {
        id: "home-02",
        title: "Star Wars Galaxy Explorer",
        version: "1.0.0",
...
function runMain() {
    ...
    main().catch((err) => {
        console.error("Error initializing BrightScript Engine:", err);
    });
}
...
function resolveElements() {
    ...
    libVersion = document.getElementById("libVersion");
...
if (document.readyState === "loading") {
    globalThis.addEventListener("DOMContentLoaded", runMain, { once: true });
} else {
    runMain();
}
...
async function main() {
    ...
    customDeviceInfo.extensions.set(brsSG, `${BASE_PATH}/lib/brs-sg.js`);
    ...
    brs.subscribe("web-app", (event, data) => {
        if (event === "loaded") {
            ...
            mountZip(`${BASE_PATH}/apps/data.zip`);
        }
    });
...
function bindFileSelectorEvents() {
    ...
    passwordDialog.addEventListener("close", () => {
        if (passwordDialog.returnValue === "ok" && fileSelector.files?.[0]) {
...
function bindDisplayEvents() {
    if (displayEventsBound) {
        return;
    }
    displayEventsBound = true;
